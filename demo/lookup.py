...
_MODEL_MAPPING: dict[ModelSelection, dict[str, str | Path]] = {
    ModelSelection.LLAVA: {
...
def get_model_info(model: ModelSelection) -> Tuple[str, str]:
    """Return the model path and spec link for a given selection.

    Args:
        model (ModelSelection): The enum entry to look up.

    Returns:
        Tuple[str, str]: A pair ``(model_path, link_to_model_spec)`` where
    ...
    """
    try:
        info = _MODEL_MAPPING[model]
    except KeyError as e:
        raise KeyError(f'Unknown model: {model!r}') from e
    ...
    return model_path, _to_repo_relative(spec_path)
